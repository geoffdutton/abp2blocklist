...
function testRules (test, filters, expected, transformFunction, options) {
  const blockerList = new ContentBlockerList(options)
  for (let filter of filters) { blockerList.addFilter(Filter.fromText(filter)) }

  return blockerList.generateRules().then(rules => {
    if (transformFunction) { rules = transformFunction(rules) }
