...
function callLater (func) {
  return new Promise(resolve => {
    ...
    // setTimeout.
    if (typeof process !== 'undefined') { process.nextTick(call) } else { setTimeout(call, 0) }
  })
}

function async (callees, mapFunction) {
  if (!(Symbol.iterator in callees)) { callees = [callees] }
  ...
  let chain = (next, index) => {
    promise = promise.then(() => {
      if (mapFunction) { next = mapFunction(next, index) }
...
function parseDomains (domains, included, excluded) {
  ...
  for (let [domain, enabled] of domains) {
    if (domain !== '') {
      if (!enabled) { excluded.push(domain) } else if (!domains['']) { included.push(domain) }
    }
  }
}
...
function getURLSchemes (contentType) {
  ...
  if (contentType & typeMap.WEBSOCKET && contentType & typeMap.WEBRTC &&
    contentType & httpRequestTypes) { return ['[^:]+:(//)?'] }

  let urlSchemes = []

  if (contentType & typeMap.WEBSOCKET) { urlSchemes.push('wss?://') }

  if (contentType & typeMap.WEBRTC) { urlSchemes.push('stuns?:', 'turns?:') }

  if (contentType & httpRequestTypes) { urlSchemes.push('https?://') }
...
function findSubdomainsInList (domain, list) {
  ...
  for (let name of list) {
    if (name.length > suffixLength && name.slice(-suffixLength) === '.' + domain) { subdomains.push(name.slice(0, -suffixLength)) }
  }
...
function extractFilterDomains (filters) {
  ...
  for (let filter of filters) {
    ...
    if (parsed.justHostname) { domains.add(parsed.hostname) }
  }
  return domains
}
...
function parseFilterRegexpSource (text, urlScheme) {
  ...
  if (!urlScheme) { urlScheme = getURLSchemes()[0] }

  for (let i = 0; i < text.length; i++) {
    let c = text[i]

    if (hostnameFinished) { justHostname = false }

    if (hostnameStart !== null && !hostnameFinished) {
      let endingChar = (c === '*' || c === '^' ||
        c === '?' || c === '/' || c === '|')
      if (!endingChar && i !== lastIndex) { continue }
    ...
    switch (c) {
      case '*':
        if (regexp.length > 0 && i < lastIndex && text[i + 1] !== '*') { regexp.push('.*') }
        break
      case '^':
        let alphabet = 'a-z'
        ...
        if (!justHostname) { alphabet = 'A-Z' + alphabet }
        let digits = '0-9'
        ...
        if (i === 0) { regexp.push('^' + urlScheme + '(.*' + separator + ')?') } else if (i === lastIndex) { regexp.push('(' + separator + '.*)?$') } else { regexp.push(separator) }
        break
      case '|':
        if (i === 0) {
          regexp.push('^')
      ...
      default:
        if (hostnameFinished && (c >= 'a' && c <= 'z' ||
          c >= 'A' && c <= 'Z')) { canSafelyMatchAsLowercase = false }
...
function getResourceTypes (contentType) {
  ...
  if (contentType & typeMap.IMAGE) { types.push('image') }
  if (contentType & typeMap.STYLESHEET) { types.push('style-sheet') }
  if (contentType & typeMap.SCRIPT) { types.push('script') }
  if (contentType & typeMap.FONT) { types.push('font') }
  if (contentType & (typeMap.MEDIA | typeMap.OBJECT)) { types.push('media') }
  if (contentType & typeMap.POPUP) { types.push('popup') }
  if (contentType & rawRequestTypes) { types.push('raw') }
  if (contentType & typeMap.SUBDOCUMENT) { types.push('document') }
...
function excludeTopURLFromTrigger (trigger) {
  ...
  if (trigger['url-filter-is-case-sensitive']) { trigger['top-url-filter-is-case-sensitive'] = true }
}

function convertFilterAddRules (rules, filter, action, withResourceTypes,
  exceptionDomains, contentType) {
  if (!contentType) { contentType = filter.contentType }
  ...
  if (filter instanceof filterClasses.WhitelistFilter &&
    ...
    if (!(contentType & whitelistableRequestTypes)) { return }
  }
  ...
  if (parsed.canSafelyMatchAsLowercase && !filter.matchCase) { trigger['url-filter'] = trigger['url-filter'].toLowerCase() }

  if (parsed.canSafelyMatchAsLowercase || filter.matchCase) { trigger['url-filter-is-case-sensitive'] = true }
  ...
  if (exceptionDomains) { excluded = excluded.concat(exceptionDomains) }
  ...
  if (withResourceTypes) {
    ...
    if (filter instanceof filterClasses.BlockingFilter && !parsed.hostname) { resourceTypes = resourceTypes.filter(type => type !== 'document') }
  ...
  if (filter.thirdParty !== null) { trigger['load-type'] = [filter.thirdParty ? 'third-party' : 'first-party'] }
  ...
  if (included.length > 0) {
    ...
    for (let name of included) {
      ...
      if ((filter instanceof filterClasses.BlockingFilter ||
        ...
        if (!notSubdomains.includes('www')) { trigger['if-domain'].push('www.' + name) }
      } else {
        trigger['if-domain'].push('*' + name)
      }
    }
  ...
  if (needAltRules) {
    ...
    for (let altRule of makeRuleCopies(trigger, { type: action }, urlSchemes)) {
      if (addTopLevelException) { excludeTopURLFromTrigger(altRule.trigger) }
...
function convertIDSelectorsToAttributeSelectors (selector) {
  ...
  for (let i = 0; i < selector.length; i++) {
    ...
    if (chr === '\\') { // ignore escaped characters
      ...
      // look for the start of an ID
      if (chr === '"' || chr === '\'') { sep = chr } else if (start === null) {
        if (chr === '#') { start = i }
        // look for the end of the ID
      } else if (chr !== '-' && chr !== '_' &&
        (chr < '0' ||
          chr > '9' && chr < 'A' ||
          chr > 'Z' && chr < 'a' ||
...
function addCSSRules (rules, selectors, domain, exceptionDomains) {
  ...
  exceptionDomains.forEach(name => {
    ...
    if (!domain || name.substr(-domain.length - 1) === '.' + domain) { unlessDomain.push('*' + name) }
  })

  while (selectors.length) {
    let selector = selectors.splice(0, selectorLimit).join(', ')

    // Ignore uBlock Origin resource injections
    if (selector.indexOf('script:') === 0 || selector.indexOf('+js') === 0) {
      break
    }
    ...
    if (unlessDomain.length > 0) { rule.trigger['unless-domain'] = unlessDomain }
...
function closeMatch (s, t) {
  ...
  if (diff === 0) {
    ...
    if (diff > 1) { edit.endIndex = s.length - j }
  } else {
    edit = {
      type: 'insert',
      index: i
  ...
  for (let i = index, j = i + 1; j < limit; j++) {
    ...
    if (edit) {
      ...
      if (multiEdit) {
        ...
        if (!ruleInfo.multiEditMatch) { ruleInfo.multiEditMatch = match }
      } else {
        // For single character edits, multiple rules can be merged into
        // one. e.g. "ad", "ads", and "adv" can be merged into "ad[sv]?".
        if (!ruleInfo.matches) { ruleInfo.matches = new Array(urlFilter.length) }
        ...
        if (!ruleInfo.bestMatches ||
          matchesForIndex.length > ruleInfo.bestMatches.length) { ruleInfo.bestMatches = matchesForIndex }
      }
    }
  }
}
      ...
      if (!multiEdit) {
        ...
        for (let match of best) {
          if (match.edit.type === 'delete') {
            ...
            // literal hyphen.
            if (character === '-') { characters.unshift(character) } else { characters.push(character) }
          }
...
function mergeRulesByArrayProperty (rulesInfo, propertyType, property) {
  if (rulesInfo.length <= 1) { return }

  let valueSet = new Set(rulesInfo[0].rule[propertyType][property])

  for (let i = 1; i < rulesInfo.length; i++) {
    for (let value of rulesInfo[i].rule[propertyType][property] || []) { valueSet.add(value) }

    rulesInfo[i].merged = true
  }

  if (valueSet.size > 0) { rulesInfo[0].rule[propertyType][property] = Array.from(valueSet) }
...
function groupRulesByMergeableProperty (rulesInfo, propertyType, property) {
  ...
  for (let ruleInfo of rulesInfo) {
    ...
    if (mergeableRulesInfo) { mergeableRulesInfo.push(ruleInfo) } else { mergeableRulesInfoByGroup.set(groupKey, [ruleInfo]) }
  }
...
ContentBlockerList.prototype.addFilter = function (filter) {
  ...
  if (filter instanceof filterClasses.RegExpFilter &&
    filter.pattern === null) { return }

  if (filter instanceof filterClasses.BlockingFilter) { this.requestFilters.push(filter) }

  if (filter instanceof filterClasses.WhitelistFilter) {
    if (filter.contentType & (typeMap.DOCUMENT | whitelistableRequestTypes)) { this.requestExceptions.push(filter) }

    if (filter.contentType & typeMap.GENERICBLOCK) { this.genericblockExceptions.push(filter) }

    if (filter.contentType & typeMap.ELEMHIDE) { this.elemhideExceptions.push(filter) } else if (filter.contentType & typeMap.GENERICHIDE) { this.generichideExceptions.push(filter) }
  }

  if (filter instanceof filterClasses.ElemHideFilter) { this.elemhideFilters.push(filter) }

  if (filter instanceof filterClasses.ElemHideException) {
    let domains = this.elemhideSelectorExceptions[filter.selector]
    if (!domains) { domains = this.elemhideSelectorExceptions[filter.selector] = [] }
...
ContentBlockerList.prototype.generateRules = function () {
  ...
  for (let filter of this.genericblockExceptions) {
    let parsed = parseFilterRegexpSource(filter.pattern)
    if (parsed.hostname) { requestFilterExceptionDomains.push(parsed.hostname) }
  }
  ...
  return async(ruleGroups, (group, index) => () => {
    let next = () => {
      if (index === ruleGroups.length - 1) { return ruleGroups.reduce((all, rules) => all.concat(rules), []) }
    }
